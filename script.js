...
var layout = {
  ...
  scrollZoom: false,
  editable: false,
  title: 'Collection of distributions at the chosen sample size',
  xaxis: {
    fixedrange: true,
    title: '',
    tickmode: 'array',
...
function updatePlot() {
  ...
  for (let i = 0; i < l; i++) {
    const ddx = dx[i];
    setTimeout(() => shiftData(xNew, ddx, yNew, 0), 10); // change trendNum to 0
  }
  setTimeout(() => addCI(xNew, yNew, ucl,maxy,0),2000);
  for (let i = 0; i < l; i++) {
    const ddx = dx[i]
    setTimeout(() => shiftData(xNew, -ddx, yNew, 1), 1000); // change trendNum to 1
  }
  var maxy = Math.max(...yNew);
  
  setTimeout(() => addCI(xNew, yNew, -ucl,maxy,1),3000);
